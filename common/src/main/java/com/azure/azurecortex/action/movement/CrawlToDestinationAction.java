...
public class CrawlToDestinationAction<E extends Mob, G> implements Action<E, G> {
    ...
    private Vec3 computeCrawlVelocity(E agent, Vec3 waypointCenter, double speed) {
        ...
        if (horizontalToWaypoint.lengthSqr() > 0.01D) {
            ...
            if (corrected.horizontalDistanceSqr() < 0.0001D) {
                ...
                if (agent.level().noCollision(agent, raisedBox)) {
    ...
    private Vec3 removeBlockedHorizontalComponents(E agent, Vec3 desired) {
        ...
        if (Math.abs(x) > 0.0001D) {
            ...
            if (!level.noCollision(agent, xProbe)) {
                x = 0.0D;
            }
        }
        ...
        if (Math.abs(z) > 0.0001D) {
            ...
            if (!level.noCollision(agent, zProbe)) {
                z = 0.0D;
            }
        }
    ...
    private Vec3 findCornerEscapeStepUpVelocity(E agent, Vec3 desiredHorizontal, double speed) {
        ...
        for (var candidate : candidates) {
            ...
            if (!level.noCollision(agent, probeBox)) {
                continue;
            }
    ...
    private Vec3 findNearestWallDirection(E agent) {
        ...
        for (var dir : dirs) {
            var hitCurrent = !level.noCollision(agent, box.move(dir.scale(probe)));
            var hitStanding = !level.noCollision(agent, standingBox.move(dir.scale(probe)));
            if (hitCurrent || hitStanding) {
                var dist = probe;
                for (var d = 0.1D; d <= probe; d += 0.1D) {
                    if (!level.noCollision(agent, box.move(dir.scale(d)))) {
                        dist = d;
                        break;
                    }
                }
