...
public class CrawlToDestinationAction<E extends Mob, G> implements Action<E, G> {
    ...
    public ActionOutcome<G> tick(E agent, Blackboard blackboard, CooldownTracker cooldowns) {
        ...
        // TEMPORARY DIAGNOSTIC — spawns an unmistakable marker above the agent's head, distinct from
        // CrawlTraversalEvaluator's own path-trail particles (which reuse FLAME/HAPPY_VILLAGER and caused visual
        // confusion the first time around): WITCH (purple swirl) = wallCrawling true this tick, TOTEM_OF_UNDYING
        // (bright gold/white burst) = ground branch AND the jump condition IS firing this tick. Nothing is spawned
        // when on the ground branch without the jump firing, so "no marker at all while stalled at the ledge" is
        // itself the answer. Gated on enablePathfindingDebug — remove once the stall is diagnosed.
    ...
    private Vec3 computeGroundVelocity(E agent, Vec3 waypointCenter, double speed) {
        ...
        if (agent.onGround() && isLowStepAhead(agent, forward)) {
