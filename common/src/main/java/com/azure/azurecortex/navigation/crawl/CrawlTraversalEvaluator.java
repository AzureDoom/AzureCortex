...
public final class CrawlTraversalEvaluator implements Pathfinder, TraversalEvaluator {
    ...
    public static boolean canOccupySlim(Level level, Mob mob, Vec3 feetPos) {
        return level.noBlockCollision(mob, slimCrawlBox(mob, feetPos));
    }
    ...
    /**
     * Computes climb clearance for {@code feet} via a fresh AABB {@code noBlockCollision} sweep. Adjacent A* nodes
     * re-derive the same feet positions constantly, so callers should go through
     * {@link PathNodeCache#hasClimbClearance} rather than calling this directly — see that method's memoization.
     */
