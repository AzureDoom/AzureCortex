...
public final class CrawlController implements NavigationHandler {
     ...
     * {@link Mob#getDefaultDimensions} to report a squeezed, wall-hugging profile while
     * {@link CrawlCapability#isWallCrawling()} is {@code true} (a wider/taller standing mob flattening against a
     * surface), but {@code Entity#dimensions} is a cached value that only recomputes when something calls
     * {@code refreshDimensions()} — it does not notice on its own that the flag it depends on just changed. A mob that
     * lazily refreshes dimensions on some unrelated periodic tick (e.g. alongside a slow growth-scale update) could
     * otherwise keep colliding with its full standing-size bounding box for several ticks after
     * {@link CrawlCapability#isWallCrawling()} already reports {@code true} — long enough to snag on a block that
     * protrudes from an otherwise flat cliff face, since the pathfinder validated that node against the smaller
     * crawl-profile footprint the mob was supposed to have at that instant, not the stale larger one it was actually
     * still colliding with. Forcing the refresh right at the transition keeps the real collision box and the profile
     * pathfinding assumed in sync.
     */
